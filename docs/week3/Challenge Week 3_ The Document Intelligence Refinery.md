...
| So what does “market validation is unambiguous” mean? It means: • Investors are funding multiple companies in the same space • The problem is painful for enterprises • Companies are willing to pay for a solution • This is not an academic exercise In simple terms: If many well funded startups attack the same problem, it signals real demand. For you, this means: Document intelligence is not a side topic. It is a major business opportunity.  |
| :---- |
...
| TO clarify the above  1, You must design extraction that preserves: • Reading order • Table structure • Section hierarchy • Bounding boxes 2, You must chunk by logical document units, not token size. • Table stays intact • Caption stays with figure • Numbered list stays together • Section header stays as metadata In easy words: • Keep related content together. • Do not split things that depend on each other. • Do not mix unrelated ideas in one chunk. 3, Every extracted fact must carry: • Document name • Page number • Bounding box coordinates • Content hash     \-always know where exactly that specific information is comming from  If you cannot open the PDF and point to the exact place, your system fails enterprise standards. |
| :---- |
...
| Stages to be performed  Stage 1 Triage agent decides extraction strategy. Stage 2 Extraction tools • MinerU • Docling • Marker Stage 3 Chunking • Chunkr or your custom chunking engine Stage 4 Index • PageIndex builds navigation tree Stage 5 LLM reasoning Or Those five tools map to those five pipeline stages. You are building a pipeline. Each stage has a job. The tools help you implement those jobs. Stage 1\. Document Triage Agent GoalUnderstand the document before processing it. Tasks • Detect scanned vs digital• Detect layout complexity• Detect language• Estimate extraction cost Tools Usually simple Python tools like: • pdfplumber• pymupdf No MinerU or Docling yet. This stage decides which extractor to use. Stage 2\. Structure Extraction Layer GoalExtract structured content from the document. This is where most of those frameworks appear. Possible tools • MinerU• Docling• Marker They extract: • text blocks• tables• figures• bounding boxes• reading order Output becomes your normalized document structure. Stage 3\. Semantic Chunking Engine GoalSplit extracted content into logical units. Instead of random token chunks, you produce chunks like: • one table• one paragraph• one figure with caption• one list Tools Inspired by: • Chunkr But most students implement their own ChunkingEngine using rules. Stage 4\. Provenance Index Builder GoalBuild navigation and traceability. Two things happen here. Build document section tree Track source location Tool inspiration • PageIndex Output includes • section tree• page numbers• bounding boxes• content hashes Stage 5\. Query Interface Agent GoalAllow users to ask questions. This stage uses LLMs. This is where Ollama or cloud models come in. Example tasks • query planning• section navigation• answer generation• citation formatting Example model usage Local models via Ollama: • llama3• qwen So the relation is clear. Extraction tools handle documents.Chunking organizes content.Indexing enables navigation.LLMs answer questions. Pipeline summary Documents → extraction tools → structured document → chunking → index → LLM query system. |
| ----- |
...
| Stage 1 E2E Entry starts from ingest in cli.py, which calls the pipeline in pipeline.py. The pipeline calls triage\_only; for PDFs it runs TriageAgent.run in triage.py, for non-PDFs it builds an equivalent profile in file\_types.py. Triage computes metrics (char density, whitespace, image ratio, table signal, column signal, form fields, text sample), then detects language+confidence. classify\_profile maps those metrics to origin\_type, layout\_complexity, domain\_hint, and estimated\_extraction\_cost. It creates a DocumentProfile model defined in models.py. The profile is saved to .refinery/profiles/{doc\_id}.json via ArtifactStore, then returned to the pipeline. Downstream extraction strategy uses this profile to decide fast text vs layout-aware vs vision-heavy path, so Stage 1 is the routing control point. |
| :---- |
