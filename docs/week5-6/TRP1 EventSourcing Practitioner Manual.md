...
| Note  | The consistency trap: you might default to inline projections because they're simpler. But inline projections mean every writer pays the cost of updating every projection. As projections multiply, write latency grows proportionally. Production systems almost always move critical read models to async with tight SLOs.  |
| :---- | :---- |
